...
flags.DEFINE_string("dataset", None, "", required=True)
...
flags.DEFINE_integer("nsteps", 8, "dm steps")
...
def predict(model, image, n, **kwargs):
    ...
    if FLAGS.nsteps > 1:
        samples = dm_sampler(
            ...
            (n, 512, 512, 2),
            solver=FLAGS.solver, num_steps=FLAGS.nsteps, discretization=FLAGS.discretization,
            **kwargs,
        )
    else:
        sigma = 80.
...
def get_ds(name):
    if Path(name).exists() and Path(name).is_dir():
        def _ds_gen():
            ...
            data_path = Path(name)
            for mask_fn in (data_path).glob("*_label.tif"):
                mask = imageio.imread(mask_fn).astype("uint16")
                img_name = mask_fn.name.replace("_label","")
                img = imageio.imread(data_path/img_name)
                yield dict(
...
def main(_):
    ...
    ds = get_ds(FLAGS.dataset)
    dst_dir = Path(FLAGS.logpath)/Path(FLAGS.dataset).name
